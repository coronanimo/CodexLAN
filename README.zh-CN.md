...
推理摘要、命令、增量输出、执行时间、上下文占用、账号限额、计划进度和实时文件差异都会显示出来。你能判断任务是在正常工作、卡在命令上，还是已经改了不该改的文件。

### 本地环境不用搬家

Codex 继续使用服务端电脑上的仓库、Shell、工具链、数据和凭据。网页可以上传附件、粘贴图片、预览文件和下载结果，但真正的执行环境始终只有一套。
